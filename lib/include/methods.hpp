...
enum class Method { Dichotomy, GoldenSection, Fibonacci, Parabolas, Brent };
...
public:
    ...
    bool forward() {
        if (check_equals(x_2, x_1) || check_equals(x_3, x_1) ||
            check_equals(x_3, x_2)) {
            return false;
        }
        a = (f_x3 -
    ...
    std::vector<IterationData<T>*> get_data() {
        std::vector<IterationData<T>*> res = {
            new IterationInterval<T>(x_1, x_3),
            new IterationInterval<T>(x_1, x_2), new IterationPoint<T>(u, f_u)};
        if (check_valid(a) && check_valid(b) && check_valid(c)) {
            res.push_back(new IterationFunction<T>(new StdFunction<T>(
...
public:
    ...
    bool forward() {
        ...
        if (fabs(e) > tol1) {
            ...
            T e_temp = e;
            e = d;
            if (fabs(p) >= fabs(0.5 * q * e_temp) || p <= q * (a - x) ||
                p >= q * (b - x)) {
                d = golden_sec_const * (e = (x >= xm ? a - x : b - x));
            } else {
                d = p / q;
                u = x + d;
                if (u - a < tol2 || b - u < tol2) d = xm - x;
            }
        } else {
            d = golden_sec_const * (e = (x >= xm ? a - x : b - x));
        }
    ...
    std::vector<IterationData<T>*> get_data() {
        std::vector<IterationData<T>*> res = {new IterationInterval<T>(a, b),
                                              new IterationPoint<T>(u, fu)};
        T f_x3 = fx;
        T f_x2 = fv;
        T f_x1 = fw;
        T x_3 = x;
        ...
        T _a = (f_x3 -
                (x_3 * (f_x2 - f_x1) + x_2 * f_x1 - x_1 * f_x2) / (x_2 - x_1)) /
               (x_3 * (x_3 - x_1 - x_2) + x_1 * x_2);
        T _b = (f_x2 - f_x1) / (x_2 - x_1) - _a * (x_1 + x_2);
        T _c = (x_2 * f_x1 - x_1 * f_x2) / (x_2 - x_1) + _a * x_1 * x_2;
        if (check_valid(_a) && check_valid(_b) && check_valid(_c)) {
            auto func = new StdFunction<T>(
                [=](T x) { return _a * x * x + _b * x + _c; });
            res.push_back(new IterationFunction<T>(func));
        }
        return res;
    }
};
